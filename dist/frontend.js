...
function setup(ctx) {
  ...
  function renderTextareaControl(slot, value, onChange, ariaLabel) {
    ...
    if (components?.mountTextArea) {
      const handle = components.mountTextArea(slot, { value, rows: 5, ariaLabel, onChange });
      activeHandles.push(handle);
      return;
    }
  ...
  function renderDirectorChannel(shell) {
    ...
    if (activeScenePanel === "lw-director-core-note")
      paper1.classList.add("is-scene-active");
    ...
    if (activeScenePanel === "lw-director-model-note")
      paper2.classList.add("is-scene-active");
    ...
    const appendCard = (title, className, ...children) => {
      ...
      if (activeScenePanel === className)
        paper.classList.add("is-scene-active");
    ...
    appendCard("System Template", "lw-director-system-note", textareaField("System template", draft.systemTemplate, (value) => updateDraft({ systemTemplate: value }), "Available variables: {{prompt}}, {{generationType}}, {{chatId}}, {{connectionId}}, {{timestamp}}, {{maxDirectiveChars}}, {{user}}, {{char}}."));
    appendCard("User Template", "lw-director-user-note", textareaField("User template", draft.userTemplate, (value) => updateDraft({ userTemplate: value })));
    const runsPaper = createElement("div", "lw-paper lw-scene-popup lw-runs-note");
    if (activeScenePanel === "lw-runs-note")
      runsPaper.classList.add("is-scene-active");
  ...
  function renderWorldAgentChannel(shell) {
    ...
    if (activeScenePanel === "lw-world-clock-note")
      clockPanel.classList.add("is-scene-active");
    ...
    if (activeScenePanel === "lw-world-config-note")
      paper1.classList.add("is-scene-active");
    paper1.appendChild(createCloseBtn());
    const head1 = createElement("div", "lw-panel-head");
    ...
    if (activeScenePanel === "lw-world-params-note")
      paper2.classList.add("is-scene-active");
    paper2.appendChild(createCloseBtn());
    const head2 = createElement("div", "lw-panel-head");
    ...
    if (activeScenePanel === "lw-world-templates-note")
      paper3.classList.add("is-scene-active");
    ...
    if (activeScenePanel === "lw-world-state-note")
      statePaper.classList.add("is-scene-active");
    ...
    if (state?.worldState?.updatedAt)
      stateHead.appendChild(createElement("span", "lw-muted", `Updated ${formatTime(state.worldState.updatedAt)}`));
    ...
    if (activeScenePanel === "lw-world-schedule-note")
      schedulePaper.classList.add("is-scene-active");
    ...
    if (activeScenePanel === "lw-world-runs-note")
      runsPaper.classList.add("is-scene-active");
    runsPaper.appendChild(createCloseBtn());
    const runsHead = createElement("div", "lw-panel-head");
  ...
  function renderGhibliScene(stage) {
    ...
    totoro.addEventListener("click", () => {
      activeScenePanel = "lw-world-config-note";
      render();
    });
  ...
  function renderSettingsModal() {
    ...
    try {
      ...
      renderBanners(stage);
      if (activeChannel === "director")
        renderDirectorChannel(stage);
      else
        renderWorldAgentChannel(stage);
