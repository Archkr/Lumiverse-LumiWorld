...
Lumiverse announces each generation ID before prompt assembly. LumiWorld uses that
ID to match the staged update to its completion event. A late end or stop from an
older generation cannot commit or discard the newer update. When a new generation
replaces one still running in the same chat, the older staged update is dropped.
If the start event is unavailable, LumiWorld skips the scene-state update rather
than associating it with an unverified reply.
