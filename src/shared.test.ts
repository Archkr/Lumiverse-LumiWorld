...
describe("settings normalization", () => {
  ...
  test("keeps an explicit empty generation selection", () => {
    expect(normalizeSettings({ generationTypes: [] }).generationTypes).toEqual([]);
    expect(normalizeSettings({ generationTypes: ["quiet"] }).generationTypes).toEqual([]);
    expect(normalizeSettings({}).generationTypes).toEqual(DEFAULT_SETTINGS.generationTypes);
  });
});

describe("operation locks", () => {
  test("allows unrelated chats while blocking duplicate Director work", () => {
...
describe("generation gating", () => {
  ...
  test("does not intercept when every reply type is unchecked", () => {
    const settings = normalizeSettings({ enabled: true, generationTypes: [] });
    for (const type of ["normal", "continue", "regenerate", "swipe", "impersonate"]) {
      expect(shouldInterceptGeneration(settings, type).intercept).toBe(false);
    }
  });
});
