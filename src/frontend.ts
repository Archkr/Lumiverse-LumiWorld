...
import { DEFAULT_SETTINGS, VISIBLE_GENERATION_TYPES, normalizeSettings, type LumiWorldSettings, type RunLogEntry, type ConnectionOption } from "./shared";
...
export function directorRuns(runs: RunLogEntry[], limit = 5): RunLogEntry[] {
  return runs.filter((run) => run.channel !== "world_agent")
...
export function setup(ctx: SpindleFrontendContext) {
  ...
  function updateTestButton(): void {
    ...
    if (node) { node.disabled = testPending || !canTest(); node.textContent = testPending ? "Testing…" : "Test Director"; }
  }

  function renderActivity(): void {
    const target = drawer.root.querySelector<HTMLElement>("[data-lw-activity]");
    if (!target) return;
    target.replaceChildren();
    const runs = directorRuns(state?.runs ?? []);
    if (!runs.length) { target.appendChild(el("div", "lw-empty", "No Director activity yet.")); return; }
    for (const run of runs) {
      ...
      if (detail) row.append(el("div", "lw-run-detail", detail));
      target.append(row);
    }
  }
  ...
  function render(): void {
    ...
    const brand = el("div", "lw-brand"); const icon = el("div", "lw-icon"); icon.innerHTML = ICON;
    const title = el("div"); title.append(el("h1", "lw-title", "Director"), el("div", "lw-version", `LumiWorld v${VERSION}`));
    brand.append(icon, title);
    const statuses = el("div", "lw-statuses");
    const directorStatus = el("span", "lw-badge"); directorStatus.dataset.lwDirectorStatus = "";
    const saveStatus = el("span", "lw-badge"); saveStatus.dataset.lwSaveStatus = "";
    statuses.append(directorStatus, saveStatus);
    header.append(brand, statuses); shell.append(header);
    ...
    updateWarnings();
    if (!state) { shell.append(el("div", "lw-loading", "Loading Director settings…")); updateDirectorStatus(); updateSaveStatus(); renderNotice(); return; }
    ...
    const test = button("Test Director", testDirector, true); test.dataset.lwTest = ""; actions.append(test);
    const retry = button("Retry save", () => scheduleSave(0)); retry.dataset.lwRetry = ""; actions.append(retry);
    core.append(actions); shell.append(core);

    const runsCard = el("section", "lw-card"); runsCard.append(el("h2", undefined, "Recent activity"));
    const activity = el("div", "lw-activity"); activity.dataset.lwActivity = ""; runsCard.append(activity); shell.append(runsCard);

    const generation = el("section", "lw-card"); generation.append(el("h2", undefined, "Runs on"));
    ...
    for (const type of VISIBLE_GENERATION_TYPES) {
      ...
      input.addEventListener("change", () => mutate({ generationTypes: input.checked
        ? [...draft.generationTypes, type] : draft.generationTypes.filter((item) => item !== type) }, true));
      label.append(input, document.createTextNode(LABELS[type])); options.append(label);
    }
    generation.append(options); shell.append(generation);

    const context = el("section", "lw-card lw-stack"); context.append(el("h2", undefined, "Director context"));
    context.append(
      switchField("Activated World Info", draft.includeWorldInfoEntries, (includeWorldInfoEntries) => mutate({ includeWorldInfoEntries }), "Send activated entries only to the Director."),
    ...
    const notes = el("section", "lw-card lw-stack"); notes.append(el("h2", undefined, "Additional notes"),
      textAreaField("Private notes", "additionalNotes", draft.additionalNotes, "Extra guidance sent only to the Director.")); shell.append(notes);
    ...
    templates.append(templateBody); shell.append(templates);

    flushMounts(); updateDirectorStatus(); updateSaveStatus(); updateTestButton(); renderActivity(); renderNotice();
  }
  ...
  cleanups.push(ctx.onBackendMessage((payload) => {
    ...
    if (message.type === "state") {
      ...
      if (drawer.root.contains(document.activeElement) && document.activeElement?.matches("input,textarea,select,[role=combobox]")) {
        renderActivity(); updateWarnings(); updateDirectorStatus(); updateSaveStatus(); updateTestButton();
      } else render();
      return;
    }
